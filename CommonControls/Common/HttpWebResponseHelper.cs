...
namespace Common
{
    public class HttpWebResponseHelper
    {
        ...
        public static HttpWebResponse CreatePostHttpResponse(string url, string content, int? timeout, string userAgent, Encoding requestEncoding, CookieContainer container, string referer, string method = "POST", string contentType = "application/json; charset=UTF-8")
        {
            HttpWebResponse response = null;
            ...
            {
                byte[] data = requestEncoding.GetBytes(content.ToString());
                request.ContentLength = data.Length;
                using (Stream stream = request.GetRequestStream())
                {
                    stream.Write(data, 0, data.Length);
        ...
        public static string PostJson(string url, string json)
        {
            ...
            try
            {
                HttpWebResponse response = CreatePostHttpResponse(url, json, null, null, null, null, "");
                result = GetResponseString(response);
            }
            catch (Exception ex)
            {
        ...
        public static string PutJson(string url, string json)
        {
            ...
            try
            {
                HttpWebResponse response = CreatePostHttpResponse(url, json, null, null, null, null, "", "PUT");
                result = GetResponseString(response);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public static string DeleteString(string url, string content)
        {
            string result = string.Empty;
            try
            {
                HttpWebResponse response = CreatePostHttpResponse(url, content, null, null, null, null, "", "DELETE");
                result = GetResponseString(response);
            }
            catch (Exception ex)
            {
        ...
        public static string UploadFile(string url, string filePath)
        {
            ...
            wc.Headers.Add("Content-Type", "application/octet-stream");//application/x-www-form-urlencoded
            byte[] recData = wc.UploadFile(new Uri(url), "POST", filePath);
