...
namespace Common
{
    ...
    public sealed class CommonHelper
    {
        ...
        public static void Replace<T>(ref T oldEntity, T newEntity)
            where T : class
        {
            try
            {
                ...
                var oPis = oldType.GetProperties().Where(pi => pi.GetCustomAttributes(false).Any(attr => attr is System.Data.Linq.Mapping.ColumnAttribute ||
                                attr is EdmScalarPropertyAttribute));
                var nPis = newType.GetProperties().Where(pi => pi.GetCustomAttributes(false).Any(attr => attr is System.Data.Linq.Mapping.ColumnAttribute ||
                                attr is EdmScalarPropertyAttribute));

                foreach (var newPi in nPis)
                {
                    object nValue = newPi.GetValue(newEntity, null);
                    foreach (var oldPi in oPis)
                    {
                        if (oldPi.Name == newPi.Name)
                        {
                            oldPi.SetValue(oldEntity, nValue, null);
                            break;
                        }
                    }
                }
                ...
                //foreach (var newPi in newType.GetProperties())
                //{
                //    object nValue = newPi.GetValue(newEntity, null);
                //    foreach (PropertyInfo oldPi in oldType.GetProperties() )
                //    {
                //        if (oldPi.Name == newPi.Name)
                //        {
                //            //object oValue = oldPi
                ...
                //            //oldPi.SetValue(oldEntity, (object)ConvertByType(oldPi.PropertyType, oValue.ToString()), null);
                //            break;
                //        }
                //    }
                //}
    ...
    public static class Extensions
    {
        ...
        public static T Replace<T>(this T oldEntity, T newEntity)
            where T : class
        {
            CommonHelper.Replace<T>(ref oldEntity, newEntity);
