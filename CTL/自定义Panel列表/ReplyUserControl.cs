...
namespace 自定义Panel列表V1
{
    public partial class ReplyUserControl : MyPanelChild
    {
        ...
        public override void RefreshData()
        {
            ...
            {
                lblDateAndUser.Text = base.DataRow["Date"].ToString() + " " + base.DataRow["UserName"].ToString();
                lblTitle.Text = base.DataRow["Title"].ToString();

                pnlReplyDetail.Controls.Clear();
                if (ReplyData != null && ReplyData.Rows.Count > 0)
                {
                    pnlReplyDetail.Visible = true;
                    int lastTop = 0;
                    foreach (DataRow item in ReplyData.Rows)
                    {
